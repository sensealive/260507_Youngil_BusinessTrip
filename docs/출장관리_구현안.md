...
- 로그인: **Supabase가 제공하는 기본 로그인**(화면에서는 부서·이름만 고르고 비밀번호만 입력하는 UX 유지). 비밀번호는 앱이 직접 테이블에 넣지 않고 Supabase Auth가 안전하게 처리
...
- 화면: [PROJECT_CONTEXT.md](../PROJECT_CONTEXT.md)대로 **부서 → 이름 → 비밀번호**만 보여 줍니다.
- 내부: 각 직원마다 앱만 아는 **고유 로그인용 문자열**을 하나 둡니다. 형식은 이메일처럼 보이게만 하면 됩니다(예: `emp_부서코드_직원id@project.local`). 사용자는 이걸 입력하지 않고, 드롭다운에서 고른 직원에 붙은 문자열로 `signInWithPassword` 를 호출합니다.
- 비밀번호 저장·검증은 **전부 Supabase Auth**가 처리합니다. 앱이 직접 DB에 비밀번호를 넣거나 해시를 비교할 필요가 없습니다.

**처음부터 로그인 가능하게 만드는 방법(운영을 단순하게)**

1. Supabase **Authentication → Users** 에서 사용자 추가, 초기 비밀번호 설정
2. `employees` 테이블에 한 줄 추가하고, 그 사용자의 **Auth 사용자 ID**를 `auth_user_id` 에 연결
...
- `employees`: 직원 정보
  ...
  - `login_email`(또는 `login_id`): Supabase Auth에 등록한 로그인용 문자열과 **동일**하게 맞춤(드롭다운에서 이 값으로 로그인 호출)
